...
(function ($, Drupal, drupalSettings) {
  ...
  Drupal.runTimeClock = function (timeEndCallback) {
    ...
    function formatClockTime(minutes, seconds, minutes_default) {
      ...
      // 本地机考改造：按 分:秒 倒计时显示（原版只显示向上取整的分钟数）
      var timeValue = minutes + ":" + seconds;
      var timeText = 'remaining';
      if (minutes < 1) {
  ...
  function showButtonsTooltip() {
    ...
    // 本地机考改造：Notepad 按钮补 tooltip（图标字体未随存档保存，按钮此前不可见）
    $('#js-bt-notepad').tooltip({
      title: 'Notepad',
      placement: "bottom",
      trigger: "hover"
    });
    ...
    // 本地机考改造：去掉交卷按钮的 tooltip（“真实机考无此功能”提示不适用于本地卷）。
...
(function ($, Drupal, drupalSettings) {
  ...
  function clickSubmitTakeTest() {
    ...
    // Attach handlers once to re-enable button on error or completion.
    $(document).once('taketest_submit_events_reading').on('taketest:submit:error taketest:submit:complete', function(){
    ...
    button_submit.click(function () {
      ...
      button_submit.prop('disabled', true).addClass('disabled');
      var mode = drupalSettings.take_test.mode;
      if (mode == 'practice_test') {
        ...
        if (time_duration_default == 0) {
          if ($('.question-palette__item.-checked').length) {
            ...
            // Re-enable since user still needs to confirm.
            button_submit.prop('disabled', false).removeClass('disabled');
          }
          else {
  ...
  function retakeTest() {
    $('.-btn-retake').click(function (e) {
      ...
      $('#modal-time-up-no-taketest').on('hidden.bs.modal', function (e) {
        ...
        Drupal.runTimeClock(timeEndReading);
      })
    })
  }

  // Handle reading test.
  Drupal.handlerReadingTest = function () {
    if (drupalSettings.tour) {
      if (!localStorage.getItem('tour_visited_product-tour-reading-take-test-pages') && !$('body').hasClass('-full-test-mode')) {
        return;
      }
    }
  ...
  Drupal.behaviors.ReadingTakeTest = {
    attach: function (context, settings) {
      window.addEventListener('load', function () {
        Drupal.handlerReadingTest()
      });
      submitTest();
      retakeTest();
    }
