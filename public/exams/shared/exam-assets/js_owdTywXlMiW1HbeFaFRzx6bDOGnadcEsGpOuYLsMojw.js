(function ($, Drupal, drupalSettings) {
  ...
  function generateQRCode(share_url, elm) {
    try {
      var qrCode = new QRCodeStyling({
        ...
        type: "svg",
        data: share_url,
        image: "/themes/iot/images/logos/IOT_ShortLogo_by_Intergreat.svg",
        dotsOptions: {
          color: "#c76478",
          type: "rounded"
        },
  ...
  Drupal.behaviors.ShareGetReward = {
    attach: function attach(context, settings) {
      // Event page.
      if (drupalSettings.iot_share_handle_shared_uri && $('.participation__item-btn').length) {
        var shareParams = new URL(drupalSettings.iot_share_handle_shared_uri).searchParams;
        $('.participation__item-btn').each(function (index, elm) {
          if (!settings.user.uid) {
            elm.href = '/account/login?destination=' + window.location.pathname;
            return;
          }

          try {
            var baseUrl = new URL(elm.href);
            for (var i of shareParams.keys()) {
              if (shareParams.get(i)) {
                baseUrl.searchParams.set(i, shareParams.get(i));
              }
            }
            elm.href = baseUrl.toString();
          }
          catch (e) {
      ...
      var shareBtns = $('[data-drupal-selector="modal-giftshare"]');
      if (shareBtns.length) {
        shareBtns.once('click_to_share_and_get_reward').on('click', function (e) {
          e.preventDefault();
          var shareModal;
          try {
            shareModal = $($(this).attr('href'));
          } catch (e) {}
          // Fallback.
          if (!shareModal || !shareModal.length) {
            shareModal = $('#modal-giftshare');
          }

          if (!shareModal.length) return;

          var shareUrl = new URL(drupalSettings.iot_share_handle_shared_uri + '&uri=' + $(this).data('share-url'));
          var curUrl = new URL(window.location.href);
          if (curUrl.searchParams.has('event_id')) {
            shareUrl.searchParams.set('event_id', curUrl.searchParams.get('event_id'));
          }
          if (curUrl.searchParams.has('uid')) {
            shareUrl.searchParams.set('uid', curUrl.searchParams.get('uid'));
          }
          // Fallback. Copy url text.
          shareModal.find('[data-drupal-selector="sharing-url"]')
            .html('<div class="hidden">' + shareUrl.toString() + '</div>');
          // Generate QR code.
          generateQRCode(shareUrl.toString(), shareModal.find('[data-drupal-selector="sharing-url"]')[0]);
...
// gen Drupal js standard wrapper
(function ($, Drupal) {
  'use strict';
              ...
              $('.writing-review-page.node-type-writing .panel-body').each(function () {
                ...
                if (child.height() > 300) {
                  if (!child.next('.test-question__expand').length) {
                    child.after('<div class="test-question__expand"><a class="test-question__expand-text">' +
                      Drupal.t('Show more') + '</a></div>');
                  }
                } else {
  ...
  /**
   * Toggle show more/less functionality for bands items.
   */
  function toggleBandsShowMore() {
    if ($('.bands__show-more').length) {
      $('.bands__show-more').once('bands-toggle').click(function () {
  ...
  Drupal.behaviors.writingReviewPage = {
  ...
  Drupal.behaviors.iotCommerceModalTopup = {
    attach: function (context, settings) {
      ...
      if ($('.form-submit.btn-checkout').length) {
        if ($('#modal-enough-credit').length) {
          $('#modal-enough-credit').on('hidden.bs.modal', function (e) {
      ...
      var currencyExc = $('.exc-value .sympol').attr('data-currency');

      if (inputElement !== null) {
        inputElement.addEventListener('input', updateSuffix);

        function resizeInput() {
          mirror.textContent = inputElement.value;
        ...
        function addCreditsValue() {
          ...
          $('.credit-board .minus-value').on('click', function (event) {
            console.log(currentCreditValue);
        ...
        function calCurrency(currentCreditValue, currentExcValue) {
          ...
          if (currencyExc === 'VND') {
            currentExCurrency = (currentCreditValue * currentExcValue).toFixed(0);
          }
          currentExCurrency = addCommas(currentExCurrency);
          $('#exc-currency').text(currentExCurrency);
          $('.iot-credit').html(currentCreditValue+'<span class="symbol">USD</span>');
        }
        ...
        $("#modal-add-credit").on('shown.bs.modal', function () {
          resizeInput();
        });
      }
    },
