...
| morph 构词 | 2942 | **≤2567(决策点 D7)** | 有 morphSeed 的必跑(不限词长);>8 无 seed 但有 ECDICT root 的跑;>8 双无证据的 316 个由 LLM 自判可否生成(无法依据真实词源时弃答,不强制不编造);≤8 无 seed 放弃 |
...
- **D7 放弃口径**(book17 实测,2026-09-07 三轮收敛):缺 morph 3285 = 必跑 2251(有 morphSeed 2136,含 ≤8 的 1402 + 短语外 >8 有 ECDICT root 的 115)+ LLM 自判档 316(>8 双无证据,prompt 明示「无法依据真实词源即返回 skip」,返回 skip 记为放弃,**不能编造、不强制生成**)+ 放弃 718(≤8 无 morphSeed 698 + 短语 20)。生成上限 2567,实际产出 2251~2567 之间。生成集证据分布:seed+root 810 / 仅 seed 1326 / 仅 root 115 / 双无 316(LLM 自判)。放弃即最终态,无种子短词不回补
...
| D7 | morph 生成范围(2026-09-07 用户三轮指示收敛) | ✅ **有 morphSeed 必走 LLM 生成(不限词长);>8 有 ECDICT root 的跑;>8 双无证据的 316 个由 LLM 自判——无法依据真实词源即返回 skip,不能编造、不强制生成;词长 ≤8 且缺 morphSeed 的直接放弃**(短语词条一律放弃)。book17 口径:必跑 2251 + 自判 316 + 放弃 718,首期调用 ~6000(上限) |
