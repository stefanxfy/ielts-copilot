...
| morph 构词 | 2942 | **仅 ~1165(决策点 D7)** | 词长 ≤8 的缺失词直接放弃不生成(2026-09-07 用户指示:短词难有真词源,硬拆必编造);只跑 >8 的非短语词 |
...
- **首期**:syl + morph(约 4600 调用,按 563 库经验分批)→ 每批跑 `check-phonetic-consistency.mjs` 门禁
- **D7 放弃口径**(book17 实测):缺 morph 3285 = 生成 1165(>8 非短语;证据分布 seed+root 320 / 仅 seed 414 / 仅 root 115 / 双无 316)+ 放弃 2120(≤8 全部 2100,短语 20)。放弃集中有 morphSeed 的 1402 个种子仍留存 contentJson,后续如需可用确定性解析(存量 633 修复 A 方案)补 morph,不走 LLM
...
| 风险 | 应对 |
| --- | --- |
| LLM 调用量大(首期 ~4600) | 分批 + 断点续跑(gen-mnemonic 已幂等);LLM 配置读 config.json,失败词清单留档重跑 |
...
| D7 | morph 生成范围(2026-09-07 用户指示) | ✅ **词长 ≤8 且缺 morph 直接放弃,不生成**(含短语词条);只对 >8 的非短语缺失词跑 LLM。理由:短词难有真词源,硬拆必编造,宁缺勿滥。book17 口径:生成 1165 / 放弃 2120,首期调用降至 ~4600 |
