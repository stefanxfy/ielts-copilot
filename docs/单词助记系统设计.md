...
> **实现路径**：音节解析（v2.2 LLM 生成「音节+音素」双层落库）+ 词根词缀（LLM 批生成 + affix 特殊词入 words 表）+ 熟词拆分（LLM 批生成 + ECDICT 二次校验）。
>
...
| 认词卡点 **模糊** ~   | Hard  | 揭示中文/例句/图 + 立即依次滑出助记：**音节解析 / 词根词缀 / 熟词拆分 / 词组搭配** |
...
- 用户点"不认识/模糊/提示/查看答案"= 主动求助信号 → 系统倾囊相助：各助记区块同框呈现（v2.2 起为辐射卡）
...
> 注（v2.1 已废除）：~~音节切分运行时算，LLM 不重复输出 syllables~~ → v2.2 起音节解析由 LLM 随同一次调用产出，落库 `contentJson.syl`（见 §2.1、附录 A2）。
...
**定位**：词组搭配是**助记区块之一**，与词根词缀 / 熟词拆分**同级同位**——任何卡片在揭示后都显示，不再走独立 modal、不再限定"默写卡 Good 后弹出"。
...
| 默写卡点 **提示** / **查看答案** 后揭示 | 同样显示各助记区块，词组搭配在内                              |
...
**显示位置**：揭示后各助记区块依次呈现，词组搭配在词根词缀 / 熟词拆分之后（v2.2 起为独立辐射卡位）。样式与另两区一致（小字号、非加粗、不抢视觉）。

---

## 3. 原型设计（径向思维导图）

**交互形态**：求助揭示后，助记信息不再堆进主卡——**主卡保持认词卡原有样式与元素、居中不动**（图 / 单词 / 音节色块 / 音标 / 例句 / 中文释义 / 评分按钮），五张**助记卡以思维导图方式从主卡辐射展开**：SVG 贝塞尔连线从主卡边缘生长到各卡，落位带 stagger 弹簧动画，hover 助记卡时对应连线加粗高亮，收起时连线回缩、卡片缩回主卡。主卡不再被撑高，信息密度由辐射卡分摊。
...
**辐射态**（literature，求助后五卡 + 连线展开）：

![径向原型 · 辐射态](../prototype/vocab/card-demo/shots/radial-literature-radiated.png)

**助记卡五种**（辐射位固定；词缺某字段时对应卡不出、连线不渲染）：

| # | 辐射位 | 卡 | 内容 | 数据字段 |
| --- | --- | --- | --- | --- |
| 1 | 顶部居中 | 🔊 **音节解析** | 音节色块 + 重音标记 + 音素 chip 条（元音暖红 / 辅音靛蓝，按音节分组）+「音素细讲」抽屉（逐音素发音要领 / 字母组合 / 发音要点） | `contentJson.syl`（附录 A2） |
| 2 | 左上 | 🧩 **词根词缀 · 熟词拆分** | 前缀 / 词根 / 后缀逐段释义 + 字面义 + 熟词锚点故事（卡内子区块） | `affixBreakdown`（内含 trueWordSplit） |
| 3 | 右上 | 🔗 **词组搭配** | 2-3 条高频搭配（中英对照） | `collocations` |
| 4 | 左下 | 🌱 **派生 / 近义词** | 同族派生词 + 中文释义（原型新增卡位，与词根同源可由 LLM 同调用产出） | `derives`（原型先行，管线接入待定） |
| 5 | 右下 | ✨ **LLM 解读** | AI 一句话讲透词源 / 记忆抓手（打字机动效，P2 延期） | `llmInsight` |

**与 §2 机制的对应**：五类机制归并为五张辐射卡——词根词缀与熟词拆分合成一张形态卡（同一构词叙事），音节解析独立成卡（v2.2 双层），派生 / 近义为展示增强位。

**实现细节**（正式开发以 `prototype/vocab/card-demo/` 为直接参考）：

- 舞台 1120×680，辐射卡固定宽 300，五卡目标位见 `radial.js` 的 `MN_POS`；音节卡内容过高时卡体 `max-height` + 内部滚动兜底（不挤压主卡、不做几何钳制）
- 连线为 SVG 贝塞尔 path，`drawn` 类触发 stroke-dashoffset 生长动画；缺字段的卡与对应连线不渲染（`mnHasContent` 判定），与「缺字段用户无感知」契约一致
...
| 字段                 | 用途          | 写入时机                | 缺字段处理      |
| ------------------ | ----------- | ------------------- | ---------- |
| `syl`              | 音节+音素解析（v2.2） | **LLM 批生成，落库**（导入校验拼接约束） | 运行时规则兜底：仅字母色块，无音素层 |
...
| 阶段       | 内容                                                            | 数据依赖                | 工作量                      |
| -------- | ------------------------------------------------------------- | ------------------- | ------------------------ |
| **P1.1** | 音节解析：syl 字段（LLM 生成落库）+ 拼接校验 + 辐射卡渲染                          | LLM 批生成 + 字段扩       | 字段扩 + 校验 + 辐射卡渲染        |
...
| **P1.5** | 词组搭配（collocations + 各区块同框渲染）                                   | LLM 批生成             | 字段扩 + 渲染                 |
...
3. eval 检查 .mn-card[data-mn="syl"|"affix"|"coll"|...] 辐射卡 DOM 存在性 + 连线 .wire 渲染
...
- 单词极短（cat、run）：无 affix、无拆分、无搭配，但有 syl（单段，无音素细分也合法——单音节词 phonemes 可整段给 desc）
- 单词极长（uncharacteristically）：syl 多段（5-6 段），affix 三段（pre+root+suffix）
...
- **同一次调用的一致性**：syl 与 affixBreakdown / trueWordSplit / collocations 在同一次 LLM 调用产出，不增加请求数；若运行时另算一套音节，两套口径可能不一致
...
各区块的服务对象是同一个：**正在攻关这个难词的用户**。词组展示"这个词实际怎么用"，与词根（构词来源）、熟词（已知子词）是同一场景下的互补信息，拆成不同触发场景（v1 的"默写 Good 后弹 modal"）徒增交互分支且信息割裂——统一为揭示后各区块同框（v2.2 起为辐射卡各占一位），简单直接。
...
> 此 JSON 即 LLM 结构化输出的目标形态（与 affixBreakdown / trueWordSplit / collocations 同一次调用产出）。desc 允许留空（重复 schwa 不必每次解释），渲染时留空项不进「音素细讲」抽屉。
