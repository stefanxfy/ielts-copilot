...
>
> **v2.9 修订（2026-09-07）**：㉓ **词书导入期「例句全取」**——`sentence` 全部例句进 `contexts`（不再只收词组命中条），`examples[]` 仍只填首条供默写卡判分；无词组命中的例句 `coll` **省略**（契约由必填转可选），渲染侧 `hiColl` 已有词头兜底高亮，语境默写卡判分只读 `examples[0]` 不受影响；㉔ **新增 `morphSeed` 可空字段**——词书 remMethod 拆分原文整存，作 gen-mnemonic prompt 种子 + 血缘留档（批量与异步「补全助记」两条生成路径共用同一种子）；㉕ `derives` 词书 relWord **直转**（81% 覆盖，省 LLM 调用），LLM 只补无 relWord 的 19%。
...
>
> **v2.8 修订（2026-09-07）**：`collocations` 独立字段**回归**（v2.5 曾删除）。v2.5 删除的前提是"词组只能靠 LLM 与例句同条生成，无独立数据源"；新东方雅思词书 JSON（bookId=IELTS_3，3575 词）提供 70% 词的独立词组数据（`phrase.phrases[].pContent/pCn`），独立字段恢复价值：① 导入期直接落库，不等 LLM；② 作为 §5 prompt ④ 的 coll 候选种子（LLM 从真实高频搭配中选用，不再凭空造搭配）；③ 未命中例句的词组留在字段内作未来素材池。`contexts` 仍为渲染唯一来源（v2.5 单字段自含**不变**），新增条目带血缘标记 `src`，由导入/回填脚本按「例句包含词组」规则维护（见下）。
>
> **v2.9 修订（2026-09-07）**：**contexts 收录范围扩大**——词书导入期例句**全取**入 `contexts`（v2.8 只收词组命中条），`coll` 相应由必填转**可选**（无词组命中的例句省略 `coll`，渲染高亮兜底词头）；`morphSeed` 与 derives 直转见版本头 ㉔㉕。

**collocations 字段（v2.8，独立词组清单，渲染不直接消费）**：

```typescript
words.contentJson.collocations?: Array<{
  phrase: string; // 词组英文原文（如 "cancel out"，按数据源原样存储，比对时小写化）
  cn?: string;    // 词组中文（如 "取消；抵销"）
}>
words.contentJson.contexts[i].src?: "llm" | "example"; // 缺省视为 "llm"，向后兼容
```

**例句→真实语境关系回填规则（v2.8，导入/回填脚本执行）**：

- 匹配：`example.en` 小写化后**精确包含** `collocation.phrase` 小写化串（不做屈折展开，宁缺勿滥；展示侧高亮本就支持屈折与 ` ... ` 通配，不受影响）
- 落库：命中且 `contexts` 无同 coll（大小写不敏感）条目 → 追加 `{ coll: phrase, collZh: cn, en: example.en, cn: example.cn, audio: example.audio, src: "example" }`
- 音频复用：`src: "example"` 条目的 audio 直接指向 `examples[].audio` 同一路径（同一句话、同一音色参数，免重合成）；空则走 TTS 兜底
- 幂等：collocations 已存在跳过；同 coll 的 example 条目已存在跳过；重复运行零变更
- 与 gen-mnemonic 的关系：LLM 生成的 contexts 条目 `src` 缺省（= llm），行为不变；`--rebuild` 整体清空 contexts 重建后需**重跑回填**恢复 example 条目（collocations 字段不受 rebuild 影响）
...
//   coll    词组高亮锚点（v2.9 转可选：词书导入例句全取时，无词组命中的句子省略；渲染兜底高亮词头）
words.contentJson.contexts: Array<{
  coll?: string;   // 该例句承载的词组英文（句中高亮锚点，必须自然嵌入 en；可空 = 无词组命中）
...
// 真实语境(单字段, 词组内嵌 + 例句音频, v2.5 对齐 examples[] 契约;v2.8 增 src 血缘)
...
// 独立词组清单(v2.8 回归, 词书数据源直接落库, 渲染不直接消费)
words.contentJson.collocations: [
  { phrase: "hard to believe", cn: "很难相信" },
  { phrase: "believe it or not", cn: "信不信由你" }
]
```
...
#   - collocations（v2.8 回归）为可选字段：存在时每条含 phrase（cn 可选）；contexts[i].src ∈ {"example"} 或缺省（=llm）
...
| 时间         | 事件                                             | 位置                                |
| ---------- | ---------------------------------------------- | --------------------------------- |
| 2026-09-07 | v2.8 修订：collocations 独立字段回归（新东方词书 JSON 为数据源，70% 词覆盖）+ 例句→contexts 关系回填规则（例句精确包含词组 → 追加 `src:"example"` 条目并复用例句音频）；contexts 单字段自含渲染职责不变 | `docs/单词助记系统设计.md` + `scripts/backfill-collocations.mjs` |
| 2026-09-07 | v2.9 修订：词书导入例句全取入 contexts（coll 转可选，渲染词头兜底）+ morphSeed 种子字段 + derives relWord 直转；配套新东方 3575 词导入落地 | `docs/单词助记系统设计.md` + `docs/新东方雅思词汇3575入库计划.md` + `scripts/import-xdf-ielts.mjs` |
