...
- 助记区块（音节解析 / 词根词缀 / 熟词拆分 / 词组搭配）**全部可选显示**，缺字段自动收起该区块（用户不感知）
- 区块在**所有卡片类型**上行为一致：认词卡求助揭示后、默写卡提示/查看答案揭示后统一展示，无折叠、无 modal（音节解析卡内的「音素细讲」抽屉是**卡内信息分层**，非交互折叠——默认收起只为控制默认信息密度）
- 助记文字字号小（10-11px）、非加粗，与主词区分（不抢视觉）

### 3.1 各卡型空间核算（无折叠，全展开直出）

助记区块全展开直出，按各卡型核算高度（单卡理想高度 ≤ 400px，音节解析卡默认收起细讲抽屉）：
...
→ 四类卡全展开均在 400px 内，无需折叠；若个别超长内容(多释义/多搭配)
  溢出，卡片内部 overflow-y 滚动兜底，不做交互折叠（音节解析卡卡体
  max-height + 内部滚动，展开抽屉后同此兜底）。
```
...
| 时间         | 事件                                             | 位置                                |
| ---------- | ---------------------------------------------- | --------------------------------- |
| 2026-09-06 | v2.2 修订：音节解析升级「音节+音素」双层、完全 LLM 生成落库 contentJson.syl；径向原型 literature 双层卡跑通（SMOKE-PASS） | `docs/单词助记系统设计.md` + `prototype/vocab/card-demo/` |
