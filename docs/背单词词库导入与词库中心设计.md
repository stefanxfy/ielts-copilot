...
| ④a edge-tts | 本地 Python CLI（免 key） | `contentJson.audio.word`、`examples[].audio` | 单词 mp3 + 例句 mp3，落 `public/audio/`，库内只存路径。**音色（2026-09-03 试音定稿）：单词 = `en-US-AndrewMultilingualNeural`（男·美音，节奏最佳），例句 = `en-US-EmmaMultilingualNeural`（女·美音，韵律最佳）+ `--rate=-8%`**；Multilingual 系停顿/连读/语调显著优于经典英音 Neural 系；导入弹窗可改选（Andrew/Brian/Ava/Emma/Sonia/Libby 六选，带试听样音） |
...
```
POST /api/vocab-book/import        创建导入任务 { name, words[], genStrategy: "core"|"all"|"none", imageStyle?: "s1"|"s6"|"s8"|"s10"|"s11", voiceWord?: <音色ShortName>, voiceSent?: <音色ShortName> }，返回 202 { id, bookId, total }
GET  /api/vocab-book/import?id=    轮询任务进度（页面导入弹窗用；含 phase/各阶段计数/未命中清单）
```

imageStyle 默认 `s1`（对齐 `src/lib/vocab-image-styles.ts` 风格池）；voiceWord/voiceSent 默认 Andrew/Emma，未传时用默认，不需要新建词书级字段——音色随导入请求走，后续整书重合成用 `scripts/resynth-audio.mjs`（支持 `--voice-word`/`--voice-sent` 覆盖，文件名不变、contentJson 无需回写）。
...
- ECDICT 补全：**运行时绝不解析大 CSV**，走 `data/ecdict-index.db` 本地索引库（`scripts/build-ecdict-index.py` 一次性构建，77 万词条 point lookup）；未命中词用 ECDICT translation 二级兜底
- 音频：并发 4 + 失败补扫一轮；已存在且 >1KB 直接跳过；edge-tts SSL reset 退避重试 5 次；`--rate` 必须等号形式（`--rate=-8%`，分开传参 argparse 报 exit 2）；单词/例句双音色拆分（默认 Andrew/Emma，例句 -8% 慢速），换音色整书重合成走 `scripts/resynth-audio.mjs`
- 生图：`genStrategy` 三选一（见 §4）；`imageStyle` 风格五选带缩略图预览；单词级重生成走已有 `/api/vocab-image`
...
- 新建弹窗（**导入已并入新建**，不再有独立「导入词库」入口，粘贴/上传 txt 即导入）：粘贴 / 上传 txt 双 tab；格式提示「一行一词、# 注释、屈折自动归并」；生图策略三选；**配图风格五选（缩略图预览，image-lab 实拍样图，默认 s1）**；**发音音色选择（单词/例句分开，带试听样音按钮，默认 Andrew/Emma）**
...
| v1.1 | 2026-09-03 | 音色定稿（单词 Andrew / 例句 Emma，例句 -8%）；导入并入新建词库；弹窗新增配图风格预览五选 + 音色试听选择；API 增 imageStyle/voiceWord/voiceSent 参数；新增 scripts/resynth-audio.mjs 整书重合成脚本 |
