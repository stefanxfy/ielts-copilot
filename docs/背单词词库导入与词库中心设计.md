...
```
POST /api/vocab-book        创建导入任务 { name, words[], genStrategy: "core"|"all"|"none", imageStyle?: "s1"|"s6"|"s8"|"s10"|"s11", voiceWord?: <音色ShortName>, voiceSent?: <音色ShortName> }
...
- 音频：并发 4，已存在且 >1KB 直接跳过；edge-tts SSL reset 退避重试；单词/例句双音色拆分（默认 Andrew/Emma，例句 -8% 慢速），换音色整书重合成走 `scripts/resynth-audio.mjs`
