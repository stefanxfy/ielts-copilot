...
/** 完成页庆祝琶音:C 大调上行五音 + 收束高八度 */
const sfxCelebrate = () => {
  tone(523.25, 0.16, 0);
  tone(659.25, 0.16, 0.12);
  tone(783.99, 0.16, 0.24);
  tone(1046.5, 0.3, 0.36);
  tone(783.99, 0.14, 0.56);
  tone(1046.5, 0.42, 0.68, "triangle", 0.14);
};
...
/** 纸屑 12 片:位置/颜色/延迟/横移伪随机(固定种子式写法,避免每次渲染跳变) */
const CONFETTI: { left: number; px: number; pd: number; c: string }[] = [
  { left: 6, px: 60, pd: 0, c: "var(--primary)" },
  { left: 14, px: -40, pd: 0.5, c: "var(--success)" },
  { left: 22, px: 30, pd: 1.1, c: "var(--destructive)" },
  { left: 30, px: -70, pd: 0.3, c: "var(--primary)" },
  { left: 38, px: 45, pd: 1.6, c: "var(--muted-foreground)" },
  { left: 47, px: -30, pd: 0.8, c: "var(--success)" },
  { left: 55, px: 55, pd: 1.9, c: "var(--primary)" },
  { left: 63, px: -55, pd: 0.2, c: "var(--destructive)" },
  { left: 71, px: 35, pd: 1.3, c: "var(--primary)" },
  { left: 79, px: -45, pd: 0.6, c: "var(--success)" },
  { left: 87, px: 65, pd: 1.5, c: "var(--muted-foreground)" },
  { left: 94, px: -60, pd: 1.0, c: "var(--primary)" },
];
...
export default function LearnPage() {
  ...
  const dailyDoneNow = useMemo(() => {
    if (!data) return 0;
    const baseline = data.stats.dailyDone;
    if (baseline <= 0) {
      // 初值 0 也可能是真的零;按本日会话增量估算(认词已评 + 默写 done)
      const ratedWordIds = new Set<number>();
      for (const w of data.queue) {
        if (w.stage === "recognize" && recogRated[w.wordId] !== undefined) {
          ratedWordIds.add(w.wordId);
        } else if (w.stage === "spell" && spellStates[`${w.spellType ?? "audio"}:${w.wordId}`]?.done) {
          ratedWordIds.add(w.wordId);
        }
      }
      return ratedWordIds.size;
    }
    return baseline;
  }, [data, recogRated, spellStates]);
  const celebratedRef = useRef(false);
  useEffect(() => {
    if (!finished || !data) return;
    if (dailyDoneNow >= data.stats.dailyTarget && !celebratedRef.current) {
      celebratedRef.current = true;
      sfxCelebrate();
    }
  }, [finished, data, dailyDoneNow]);

  /* ---- 继续背新词 +10:临时放宽限额拉新队列(本轮完成页仅一次) ---- */
  const loadMore = useCallback(async () => {
    if (extraUsed || extraLoading) return;
    setExtraLoading(true);
    try {
      const resp = await fetch("/api/vocab-review?extra=10");
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
      const d = (await resp.json()) as SessionData;
      if (d.queue.length === 0) return; // 没有新词可加(计划内全部学完)
      setExtraUsed(true);
      setRecogRated({});
      setSpellStates({});
      setIdx(0);
  ...
  if (finished) {
    const dailyDoneNow = data.stats.dailyDone;
    const goalMet = dailyDoneNow >= data.stats.dailyTarget;
    return goalMet ? (
      <div className="fin-celebrate">
        <div className="fin-rays" aria-hidden="true" />
        ...
        <div className="fin-trophy">
          <TrophyIcon />
        </div>
        <span className="fin-done-badge">✓ 今日任务已达成</span>
        <h2 className="fin-title">今日背词目标达成！</h2>
        <div className="fin-target">
          今日 <b>{dailyDoneNow}</b> / {data.stats.dailyTarget} 词
          {data.stats.dailySource === "plan" ? " · 备考计划任务" : " · 自定目标"}
        </div>
        <p className="fin-sub">
          本轮完成 <b>{queue.length}</b> 词 · 今日累计复习{" "}
          <b>{data.stats.todayReviewed + todayExtra}</b> 次。
          {extraUsed
            ? "加量已用完,明天再战——记忆需要时间沉淀。"
            : "意犹未尽?可以再背 10 个新词。"}
        </p>
        {!extraUsed && (
          <>
            <button
              ...
              onClick={loadMore}
            >
              {extraLoading ? "加词中…" : "继续背新词 +10"}
            </button>
            <span className="fin-more-hint">一次加 10 个,不能贪杯 😉</span>
          </>
        )}
        <div className="flex flex-wrap items-center justify-center gap-3">
          <button
            ...
            className="press-bubble cursor-pointer rounded-full border border-border bg-secondary px-5 py-2 text-[13px] font-medium text-secondary-foreground transition-colors hover:bg-accent"
          >
            查看今日单词轨迹 →
          </button>
          <Link
      ...
      <div className="fin-celebrate fin-plain">
        <h2 className="text-xl">本轮复习完成 🎉</h2>
        <p className="fin-sub">
          共完成 <b>{queue.length}</b> 个词 · 今日已背 <b>{dailyDoneNow}</b>/{data.stats.dailyTarget} 词
          、复习 <b>{data.stats.todayReviewed + todayExtra}</b> 次
          <br />
          继续加油,今日目标还差 {data.stats.dailyTarget - dailyDoneNow} 个词;下一批到期词也由
          FSRS 按遗忘曲线调度。
        </p>
        <div className="flex flex-wrap items-center justify-center gap-3">
          <button
